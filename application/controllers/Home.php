...
class Home extends CI_Controller {
	...
	public function fetch_report()
	{
		...
		if($id == 1){
			...
			$sql ="SELECT count(id) as total, DATE_FORMAT(date_time,'%M') as value FROM documents inner join history on documents.id = history.fk_document GROUP BY DATE_FORMAT(`date_time`, '%m%') having YEAR(CURRENT_TIMESTAMP)";
		}else{
